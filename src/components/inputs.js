...
export function PitInputLabelled(props){
  return(
    <div style={{paddingTop:'20px',width:props.width?props.width:'100%',paddingBottom:'5px'}}>
        ...
        <input name={props.name} multiple={props.multiple?true:false} onChange={(e)=>props.onChange(e)} style={{height:props.height?props.height:''}} className='component_pitInput' placeholder={props.placeholder} type={props.type} value={props.value} {...props.reg}></input>
    </div>
);
}
